import torch
from torch.nn import Module, Linear, ReLU, Dropout, BCEWithLogitsLoss, CrossEntropyLoss
...
class PytorchVFAE(SupervisedPytorchBaseModel):
    ...
    def set_pu(self, pu):
        pu_dist = Bernoulli(torch.tensor(pu).to(self.device))
...
class VariationalFairAutoEncoder(Module):
    ...
    def __init__(self,
        ...
        self.discriminator = DecoderMLP(z_dim, z_dim, s_dim, activation)
    ...
    def forward(self, inputs):
        ...
        s_decoded = self.discriminator(z1_encoded)
        p_adversarial = Bernoulli(logits=s_decoded)
        log_p_adv = self.pu.log_prob(s)
        log_p_u = p_adversarial.log_prob(s)
        ...
        return self.vae_loss, self.mi_sz
...
class VFAELoss(Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        x_s = torch.cat([x, s], dim=-1)
        device = y.device
        supervised_loss = self.ce(y_pred['y_decoded'], y.squeeze().type(torch.LongTensor).to(device))
